...
Below are instructions to install, update, and use the correct python programming environment for the course. You can use another setup if you prefer, but this is the one we support.
...
## Daily use
  ...
  * From now on, **always and only** run `conda` (to install/update packages) and JupyterLab (to use your python environment) from the command line. Always first activate the `4940` virtual environment, like so:
